...
const DRIVE_CLIENT_ID =
  "616515156131-pglvg65fnlnd8d0m7coovpsoc1a40t7v.apps.googleusercontent.com";
...
function isSourceOfTruth(fileName) {
  ...
  if (
    currentUserEmail &&
    currentUserEmail.toLowerCase() === SOURCE_OF_TRUTH_EMAIL.toLowerCase()
  )
    return true;
...
function updateDriveBtn(connected) {
  ...
  if (connected) {
    ...
    if (connectedWithData) connectedWithData.style.display = "none";
    if (statusRow) {
      statusRow.style.background = "var(--bg)";
      statusRow.style.borderColor = "var(--bg2)";
    }
  }
}
...
async function findFile(name) {
  ...
  const q = encodeURIComponent(
    `name='${name}' and '${DRIVE_FOLDER_ID}' in parents and trashed=false`,
  );
  const list = await driveReq(
    "GET",
...
async function writeFile(name, content, mimeType) {
  ...
  try {
    ...
    if (!fileId)
      fileId = await createFile(name, mimeType || "application/octet-stream");
    await fetch(
...
async function checkExistingData() {
  try {
    ...
    if (dbRaw) {
      ...
      const lastDate =
        meta.lastDate ||
        meta.lastMerge?.substring(0, 10).split("-").reverse().join(".") ||
        "—";
...
function showSmartUpload(lastDate, count) {
  ...
  if (statusRow) {
    statusRow.style.background = "#f0faf4";
    statusRow.style.borderColor = "#27ae60";
  }
...
function loadParsedFromDB(db, startDate, endDate) {
  ...
  db.calls.forEach((dbCall, i) => {
    ...
    // החל overrides לפי callKey יציב
    const tempCallForKey = {
      date: dbCall.date,
      time: dbCall.time,
    ...
    const callKey =
      typeof getCallKey === "function"
        ? getCallKey(tempCallForKey)
        : [
            dbCall.date,
            dbCall.time,
            dbCall.region,
    ...
    const assignment = manualAssignments[callKey];
    ...
    let status =
      manualStatuses[callKey] !== undefined
        ? manualStatuses[callKey]
        : origStatus;
    const closingNote =
...
async function loadFromDrive() {
  ...
  try {
    ...
    console.warn("Failed loading panther-chat.txt", err);
  }
  ...
  if (!chatText) {
    alert("לא נמצא קובץ צ׳אט ב-Drive");
    return;
  }
  rawText = chatText;
  document.getElementById("analyzeBtn").disabled = false;
...
async function saveChatToDrive(text, fileName) {
  ...
  for (let i = lines.length - 1; i >= 0; i--) {
    ...
    if (m) {
      lastDate = `${m[1].padStart(2, "0")}.${m[2].padStart(2, "0")}.${m[3]}`;
      break;
    }
  ...
  try {
    ...
    allLines.forEach((line, i) => {
      if (iosRe.test(line) || andRe.test(line)) msgLineIndices.push(i);
    });
    ...
    const newCalls = parsed.calls.map((c) => {
      ...
      const lineIdx = allLines.findIndex((line) => {
        ...
        return (
          dateVariants.some((d) => line.includes(d)) &&
    ...
    console.log(
      "[saveChatToDrive] newCalls:",
      newCalls.length,
      "isSourceOfTruth:",
      isSourceOfTruth(fileName),
      "fileName:",
      fileName,
    );
    console.log(
      "[saveChatToDrive] sample rawContext:",
      newCalls[0]?.rawContext?.substring(0, 80),
    );
    const { db: mergedDb, added } = await mergeCallsIntoDB(
      newCalls,
      db,
      isSourceOfTruth(fileName),
    );
    ...
    console.log(
      `DB: נוספו ${added} קריאות חדשות, סה"כ ${mergedDb.calls.length}`,
    );
  } catch (e) {
    console.error("DB merge error:", e);
  }
}
...
async function loadOverridesFromDrive() {
  try {
    ...
    if (obj.changelog)
      manualChangelog = { ...obj.changelog, ...manualChangelog };
    saveStorage();
  } catch (e) {
...
async function saveOverridesToDrive() {
  ...
  await writeFile(
    "panther-overrides.json",
    JSON.stringify(obj),
    "application/json",
  );
  // עדכן etag כדי שהמכשיר הזה לא יזהה את עצמו כ"שינוי חדש"
  getFileEtag("panther-overrides.json").then((etag) => {
...
async function loadPhonebookFromDrive() {
  try {
    ...
    Object.values(phonebook).forEach((name) => {
      if (name && name.length > 1) volDatabase.add(name);
    });
    renderVolDb();
  } catch (e) {
    console.error("Phonebook load error:", e);
  }
}
...
async function savePhonebookToDrive() {
  ...
  volDatabase.forEach((name) => {
    if (!phonebook[name]) phonebook[name] = name;
  });
  await writeFile(
    "panther-phonebook.json",
    JSON.stringify(phonebook, null, 2),
    "application/json",
  );
}
...
function exportCSV() {
  ...
  const rows = parsedData.calls.map((c, i) => {
    const callKey =
      ...
      (typeof getCallKey === "function"
        ? getCallKey(c)
        : [c.date, c.time, c.region, c.location, c.vehicle || ""].join("||"));
    const handler = manualAssignments[callKey]
      ? manualAssignments[callKey].replace(/\|/g, ", ")
      : c.handlerClean || "";
    const status = manualStatuses[callKey] || c.origStatus || c.status;
    const rawNote =
      manualNotes[callKey] !== undefined
        ? manualNotes[callKey]
        : c.closingNote || "";
...
function exportPhonebook() {
  ...
  const a = document.createElement("a");
  a.href = url;
  a.download = "panther-phonebook.csv";
  a.click();
  URL.revokeObjectURL(url);
}
...
let manualChangelog = {}; // callKey -> [{user, action, time}]
...
async function getFileEtag(filename) {
  ...
  try {
    ...
    if (data.files && data.files.length > 0) {
      ...
      else if (email.includes("nati") || email.includes("natabar"))
        uploaderName = "נתי";
      else if (f.lastModifyingUser?.displayName)
        uploaderName = f.lastModifyingUser.displayName;
...
function showChangelog(callIndex) {
  ...
  const callKey =
    ...
    (typeof getCallKey === "function"
      ? getCallKey(c)
      : [c.date, c.time, c.region, c.location, c.vehicle || ""].join("||"));
  const log = manualChangelog[callKey] || [];
  const body = document.getElementById("changelogBody");
  if (!log.length) {
    body.innerHTML =
      '<div style="color:var(--text-muted);text-align:center;padding:20px;">אין שינויים רשומים</div>';
  } else {
    body.innerHTML = log
      .slice()
      .reverse()
...
function addChangeLog(callIndex, action) {
  ...
  const callKey =
    ...
    (typeof getCallKey === "function"
      ? getCallKey(c)
      : [c.date, c.time, c.region, c.location, c.vehicle || ""].join("||"));
  if (!manualChangelog[callKey]) manualChangelog[callKey] = [];
  const entry = {
  ...
  manualChangelog[callKey].push(entry);
}
...
function clearChangeLog(callIndex) {
  ...
  const callKey =
    ...
    (typeof getCallKey === "function"
      ? getCallKey(c)
      : [c.date, c.time, c.region, c.location, c.vehicle || ""].join("||"));
  delete manualChangelog[callKey];
}
