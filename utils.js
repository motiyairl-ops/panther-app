...
let rawText = "";
let manualAssignments = {}; // callKey -> volName
let manualStatuses = {}; // callKey -> 'cancelled'|'open'|'sab' (manual status override)
let manualNotes = {}; // callKey -> closing note text
let volDatabase = new Set();
let assigningCallIndex = -1;
let assigningCallKey = "";
let acSelectedIdx = -1;

function normalizeCallKeyPart(value) {
  return String(value || "")
    .replace(/\s+/g, " ")
    .replace(/[\u200e\u200f\u202a-\u202e]/g, "")
    .trim()
...
function getCallKey(callOrIndex) {
  const c =
    typeof callOrIndex === "number"
      ? parsedData?.calls?.[callOrIndex] || null
      : callOrIndex;

  if (!c) return "";

  return [
    normalizeCallKeyPart(c.date),
    normalizeCallKeyPart(c.time),
    normalizeCallKeyPart(c.region),
    normalizeCallKeyPart(c.location),
    normalizeCallKeyPart(c.vehicle || ""),
  ].join("||");
}
...
function isPantherGroup(text) {
  ...
  return (
    /פנתר|ידידים|מבצעי|מרחב/i.test(sample) ||
...
async function loadZipFile(file) {
  ...
  try {
    ...
    if (!JSZip) {
      alert("שגיאה: ספריית ZIP לא נטענה");
      return;
    }
...
async function processText(text, fileName, fileSize) {
  ...
  document.getElementById("dzSub").textContent =
    `✓ נטען: ${fileName} (${(fileSize / 1024).toFixed(0)} KB)`;
  ...
  if (driveAccessToken) {
    try {
      ...
      console.log(
        "[processText] saved. pantherDB calls:",
        pantherDB?.calls?.length,
      );
      document.getElementById("dzSub").textContent =
        `✅ ${fileName} — נשמר ב-Drive`;
    } catch (e) {
      console.error("processText save error:", e);
    }
  } else {
  ...
  if (dates.length) {
    ...
    const us = document.getElementById("uploadStartDate");
    if (us) us.value = fmtDate(weekStart);
    const ue = document.getElementById("uploadEndDate");
    if (ue) ue.value = fmtDate(weekEnd);
    // sync header datepicker buttons
    ["btnUploadStart", "btnStart"].forEach((id) => {
      const el = document.getElementById(id);
      if (el) el.textContent = "📅 " + formatDisplayDate(weekStart);
    });
    ["btnUploadEnd", "btnEnd"].forEach((id) => {
      const el = document.getElementById(id);
      if (el) el.textContent = "📅 " + formatDisplayDate(weekEnd);
    });
    const ms = document.getElementById("mobileStartDate");
    if (ms) ms.value = fmtDate(weekStart);
    const me = document.getElementById("mobileEndDate");
    if (me) me.value = fmtDate(weekEnd);
  }
  ...
  if (driveAccessToken) {
    saveChatToDrive(rawText).then(() => {
      ...
      const msgRe =
        /^\[(\d{1,2}\.\d{1,2}\.\d{4})|^(\d{1,2}\.\d{1,2}\.\d{4}),\s*\d{1,2}:\d{2}\s*-/;
      let lastDate = "";
      ...
      showSmartUpload(
        pantherDB ? pantherDB.meta?.lastDate || lastDate : lastDate,
        pantherDB ? pantherDB.calls.length : lines.length,
      );
...
async function analyze() {
  ...
  if (pantherDB && pantherDB.calls && pantherDB.calls.length > 0) {
    let s = document.getElementById("weekStart").value,
      e = document.getElementById("weekEnd").value;
    if (!s || !e) {
...
function renderDashboard(s, e) {
  ...
  calls.forEach((c) => {
    c.callKey = getCallKey(c);

    // Always reset to original parsed values first
    c.status = c.origStatus;
    c.handlerClean = c.origHandlerClean;

    if (manualAssignments[c.callKey] !== undefined) {
      const assignment = manualAssignments[c.callKey];
    ...
    if (manualStatuses[c.callKey] !== undefined) {
      c.status = manualStatuses[c.callKey];
      if (c.status !== "sab") c.handlerClean = "";
    }

    // Apply manual closing note
    if (manualNotes[c.callKey] !== undefined) {
      c.closingNote = manualNotes[c.callKey];
    }
    // Count volunteers — split by pipe only (comma is part of names)
    if (c.status === "sab" && c.handlerClean) {
      const names = c.handlerClean
        .split("|")
        .map((n) => n.trim())
        .filter((n) => n.length >= 3);
      names.forEach((name) => {
        ...
        const canonicalKey =
          existingKey && existingKey.length >= name.length ? existingKey : name;
        ...
        if (!volunteers[canonicalKey])
          volunteers[canonicalKey] = {
            count: 0,
            region: c.region,
            callIndices: [],
          };
  ...
  const successfulRescueCount = Object.values(volunteers).reduce(
    (sum, v) => sum + (v.count || 0),
    0,
  );
  ...
  if (s && e)
    sectionTitle += ` · ${s.split("-").reverse().join(".")}–${e.split("-").reverse().join(".")}`;
  ...
  document.getElementById("volList").innerHTML = vs.length
    ? vs
        .map(([name, data], i) => {
      ...
      <div class="vol-count">${data.count}</div>
        ...
        .join("")
    : '<div class="empty-msg">לא זוהו כוננים</div>';

  const maxT = Math.max(...Object.values(regions).map((r) => r.total), 1);
  document.getElementById("regionBadge").textContent =
    `${Object.keys(regions).length} אזורים`;
  document.getElementById("regionsGrid").innerHTML = Object.entries(regions)
  ...
  sel.innerHTML =
    ...
    Object.keys(regions)
      .sort()
      .map((r) => `<option value="${r}">מרחב ${r}</option>`)
      .join("");
}

function callRowHtml(c, i) {
  const callKey = c.callKey || getCallKey(c);
  const cancelReason =
    manualNotes[callKey] !== undefined
      ? manualNotes[callKey]
      : c.closingNote
        ? formatCancelReason(c.closingNote)
        : "";
  ...
  const cancelLine =
    (c.status === "cancelled" || c.status === "transferred") && cancelReason
  ...
  const handlerLine = c.handlerClean
    ? `<div class="handler-name">
        🙋 טיפל בקריאה:
        <button class="handler-edit-btn" onclick="event.stopPropagation();openAssign(${c.index})"><strong>${c.handlerClean
          .split("|")
          .map((n) => n.trim())
          .join(", ")}</strong></button>
    ...
    : `<div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap;">
        ...
        ${c.status === "cancelled" && manualStatuses[callKey] === "cancelled" ? `<button class="btn-cancel-manual" onclick="event.stopPropagation();manualUncancel(${c.index})" style="background:rgba(39,174,96,0.12);color:var(--green);">↩ פתח מחדש</button>` : ""}
      </div>`;
...
function formatCancelReason(note) {
  ...
  if (/מישהו שם חילץ|מישהו עזר|חילצו אותו|עזרה מקומית/i.test(n))
    return 'חולץ ע"י אחר';
...
function switchTab(tab) {
  ...
  document.getElementById("tabVols").style.display =
    tab === "vols" ? "block" : "none";
  document
    .getElementById("tab-dash")
    .classList.toggle("active", tab === "dash");
  document
    .getElementById("tab-search")
    .classList.toggle("active", tab === "search");
  document
    .getElementById("tab-vols")
    .classList.toggle("active", tab === "vols");
  if (tab === "search") doSearch();
  if (tab === "vols") renderVolDb();
}
...
function doSearch() {
  ...
  const res = calls.filter((c) => {
    ...
    if (
      q &&
      !`${c.location} ${c.vehicle} ${c.handlerClean} ${c.region} ${c.date}`
        .toLowerCase()
        .includes(q)
    )
      return false;
...
function renderVolDb() {
  const q = (
    document.getElementById("volDbSearch") || { value: "" }
  ).value.toLowerCase();
  const vols = [...volDatabase]
  ...
  list.innerHTML = vols.length
    ...
    <div style="padding:12px 20px;border-bottom:1px solid var(--bg);display:flex;align-items:center;">
      <div style="font-size:14px;font-weight:600">🙋 ${v}</div>
        ...
        .join("")
    : '<div class="empty-msg">אין כוננים במאגר</div>';
}

function addVolManual() {
  const name = prompt("שם הכונן:");
  if (name && name.trim()) {
    volDatabase.add(name.trim());
    saveAll();
    renderVolDb();
  }
}
function removeVol(name) {
  volDatabase.delete(name);
  saveAll();
  renderVolDb();
}
...
function markDirty() {
  const isDirty =
    JSON.stringify(pendingHandlers) !== JSON.stringify(originalHandlers) ||
    document.getElementById("assignInput").value.trim() !== "";
  document.getElementById("assignActions").style.display = isDirty
    ? "flex"
    : "none";
  document.getElementById("assignNoChanges").style.display = isDirty
    ? "none"
    : "block";
}
...
function editCancelReason(callIndex) {
  ...
  const callKey = getCallKey(c);
  const current = manualNotes[callKey] || c.closingNote || "";
  const newNote = prompt("עריכת סיבת סיום:", current);
  if (newNote === null) return; // cancelled
  const oldNote = manualNotes[callKey];
  manualNotes[callKey] = newNote.trim();
  if (oldNote !== newNote.trim())
    addChangeLog(callIndex, "עריכת הערה: " + (newNote.trim() || "(נמחק)"));
  saveAll();
  showSyncNotice();
...
function manualCancel(callIndex) {
  const c = parsedData.calls[callIndex];
  const callKey = getCallKey(c);
  manualStatuses[callKey] = "cancelled";
  manualNotes[callKey] = manualNotes[callKey] || "בוטל ידנית";
  delete manualAssignments[callKey];
...
function openAssign(callIndex) {
  ...
  const c = parsedData.calls[callIndex];
  const callKey = getCallKey(c);
  assigningCallKey = callKey;
  document.getElementById("assignCallInfo").innerHTML =
    `<strong>📍 ${c.location}</strong><br>מרחב ${c.region} · ${c.date} ${c.time}`;
  document.getElementById("assignInput").value = "";
  acSelectedIdx = -1;
  const existing = manualAssignments[callKey] || c.handlerClean || "";
  pendingHandlers = existing
    ? existing
        .split("|")
        .map((s) => s.trim())
        .filter(Boolean)
...
async function clearAssignOverride() {
  ...
  const c = parsedData.calls[assigningCallIndex];
  const callKey = assigningCallKey || getCallKey(c);
  delete manualAssignments[callKey];
  saveAll();
  closeAssign();
  const s = document.getElementById("weekStart").value;
...
function updateAcList(q) {
  ...
  const matches = [...volDatabase]
    .filter((v) => v.toLowerCase().includes(lq))
    .sort()
    .slice(0, 8);
  document.getElementById("assignList").innerHTML = matches
    .map(
      (v, i) => `
    <div class="autocomplete-item${i === acSelectedIdx ? " selected" : ""}" onmousedown="selectVol('${v.replace(/'/g, "\\'")}')">${v}</div>`,
    )
    .join("");
  document.getElementById("assignList").style.display = matches.length
    ? "block"
    : "none";
}
...
function onAssignKey(e) {
  ...
  if (e.key === "ArrowDown") {
    ...
    if (acSelectedIdx >= 0 && items[acSelectedIdx]) {
      selectVol(items[acSelectedIdx].textContent);
    } else {
      confirmAssign();
    }
...
async function confirmAssign() {
  ...
  const c = parsedData.calls[assigningCallIndex];
  const callKey = assigningCallKey || getCallKey(c);
  const prevAssign = manualAssignments[callKey];
  manualAssignments[callKey] = combined || undefined;
  pendingHandlers.forEach((n) => volDatabase.add(n));
  if (combined !== (prevAssign || "")) {
    const prevList = prevAssign
      ? prevAssign.split("|").map((n) => n.trim())
      : [];
    ...
    if (added.length)
      addChangeLog(assigningCallIndex, "נוסף כונן: " + added.join(", "));
    if (removed.length)
      addChangeLog(assigningCallIndex, "הוסר כונן: " + removed.join(", "));
    if (!added.length && !removed.length && combined)
      addChangeLog(assigningCallIndex, "עודכן שיוך כונן");
  }
  // לוגיקת סטטוס אוטומטית
  if (combined) {
    manualStatuses[callKey] = "sab";
  } else if (prevAssign) {
    manualStatuses[callKey] = "cancelled";
    if (manualNotes[callKey] === undefined) manualNotes[callKey] = "";
  }
...
function showVolCalls(name, indices) {
  ...
  document.getElementById("volModalBody").innerHTML = volCalls
    .map((c, i) => {
      const statusBadge =
        c.status === "sab"
...
function scrollToCall(idx) {
  const call =
    parsedData && parsedData.calls
      ? parsedData.calls.find((c) => c.index === idx)
      : null;
...
document.addEventListener("mousedown", (e) => {
  document.querySelectorAll(".cal-popup.open").forEach((popup) => {
    if (
      ...
      !e.target.classList.contains("datepicker-display")
    ) {
      closeAllCals();
    }
  });
...
function renderCal(which, popupId, btnId) {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    const isStart =
      calState.selectedStart && fmtDate(calState.selectedStart) === dateStr;
    const isEnd =
      calState.selectedEnd && fmtDate(calState.selectedEnd) === dateStr;
    const inRange =
      calState.selectedStart &&
      calState.selectedEnd &&
      dateObj > calState.selectedStart &&
      dateObj < calState.selectedEnd;
...
function onMobileDateChange() {
  ...
  ["btnStart", "btnUploadStart"].forEach((id) => {
    const el = document.getElementById(id);
    if (el) el.textContent = fmt(calState.selectedStart);
  });
  ["btnEnd", "btnUploadEnd"].forEach((id) => {
    const el = document.getElementById(id);
    if (el) el.textContent = fmt(calState.selectedEnd);
  });
  reanalyze();
}
...
async function saveDatabase(db) {
  ...
  try {
    await writeFile(
      "panther-database.json",
      JSON.stringify(db),
      "application/json",
    );
    // עדכן etag אחרי כתיבה — המכשיר שמעלה לא יקבל באנר "נתונים חדשים"
    getFileEtag("panther-database.json").then((etag) => {
      if (etag) lastDbEtag = etag;
    });
  } catch (e) {
...
async function mergeCallsIntoDB(newCalls, db, updateNameDict) {
  ...
  for (const call of newCalls) {
    ...
    const matchId = existingMap.has(id)
      ? id
      : existingMap.has(altId1)
        ? altId1
        : existingMap.has(altId2)
          ? altId2
          : null;
  ...
  db.meta.lastDate =
    newCalls.length > 0
      ? newCalls[newCalls.length - 1].date
      : db.meta.lastDate || "";
